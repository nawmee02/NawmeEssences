-- ============================================================
--  NawmeEssences — product sales (% off) + per-product SEO meta
--  overrides. Adds columns to fragrances and extends the
--  upsert_product RPC with three new params.
--  Run in Supabase Dashboard → SQL Editor (re-runnable).
--  Depends on 004_admin.sql (is_admin, upsert_product).
-- ============================================================

-- ─────────────────────────────────────────
--  1. New columns on fragrances
-- ─────────────────────────────────────────
ALTER TABLE fragrances ADD COLUMN IF NOT EXISTS sale_percent int NOT NULL DEFAULT 0;
DO $$ BEGIN
  ALTER TABLE fragrances ADD CONSTRAINT fragrances_sale_percent_chk
    CHECK (sale_percent BETWEEN 0 AND 95);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

ALTER TABLE fragrances ADD COLUMN IF NOT EXISTS meta_title text;
ALTER TABLE fragrances ADD COLUMN IF NOT EXISTS meta_description text;

-- ─────────────────────────────────────────
--  2. Recreate upsert_product with 3 new trailing params.
--     Adding params changes the signature, so drop the old one first.
-- ─────────────────────────────────────────
...
BEGIN
  ...
  -- Fragrance upsert (now with sale_percent + meta overrides)
